    ...
    def act(self, env, state, demand, n1, n2):
        ...
        while allocated == 0 and path < len(pathList) and path < self.K:
            ...
            while j < len(currentPath):
                env.graph_state[env.edgesDict[str(currentPath[i]) + ':' + str(currentPath[j])]][0] -= demand
                env.graph_state[env.nodesDict[str(currentPath[i])]][5] -= demand
                env.graph_state[env.nodesDict[str(currentPath[j])]][5] -= demand
                if ((env.graph_state[env.edgesDict[str(currentPath[i]) + ':' + str(currentPath[j])]][0] < 0)
                             or (env.graph_state[env.nodesDict[str(currentPath[i])]][5] < 0)
                             or (env.graph_state[env.nodesDict[str(currentPath[j])]][5] < 0)):
                    can_allocate = 0
                    break
                i = i + 1
                j = j + 1
...
class LBAgent:
    ...
    def act(self, env, state, demand, n1, n2):
        ...
        while free_capacity < 2 and path < len(pathList) and path < self.K:
            ...
            while j < len(currentPath):
                # 提取节点可靠性
                self.node_dependability.append(env.graph_state[env.nodes[i]][4])

                # 提取边的可靠性
                edge = str(currentPath[i]) + ':' + str(currentPath[j])
                self.edge_dependability.append(env.graph_state[env.edgesDict[edge]][2])
                self.edge_length.append(env.graph_state[env.edgesDict[edge]][3])
                i = i + 1
                j = j + 1
            total_edge_length = 0
            ...
            # product now contains the product of all values in the column indexed by 2
            # 1. Iterate over pairs of nodes and check if we can allocate the demand
            i = 0
            j = 1
            while j < len(currentPath):
                ...
                env.graph_state[env.nodesDict[str(currentPath[j])]][5] -= demand
                if demand == 8:
                    if result < 0.9 or total_edge_length > 1000 or \
                            ((env.graph_state[env.edgesDict[str(currentPath[i]) + ':' + str(currentPath[j])]][0] < 0)
                             or (env.graph_state[env.nodesDict[str(currentPath[i])]][5] < 0)
                             or (env.graph_state[env.nodesDict[str(currentPath[j])]][5] < 0)):
                        can_allocate = 0
                        break
                if demand == 32:
                    if result < 0.9 or total_edge_length > 950 or \
                            ((env.graph_state[env.edgesDict[str(currentPath[i]) + ':' + str(currentPath[j])]][0] < 0)
                             or (env.graph_state[env.nodesDict[str(currentPath[i])]][5] < 0)
                             or (env.graph_state[env.nodesDict[str(currentPath[j])]][5] < 0)):
                        can_allocate = 0
                        break
                if demand == 64:
                    if result < 0.9 or total_edge_length > 900 or \
                            ((env.graph_state[env.edgesDict[str(currentPath[i]) + ':' + str(currentPath[j])]][0] < 0)
                             or (env.graph_state[env.nodesDict[str(currentPath[i])]][5] < 0)
                             or (env.graph_state[env.nodesDict[str(currentPath[j])]][5] < 0)):
                        can_allocate = 0
        ...
        # If there is just one path to allocate we allocate it there
            ...
            while allocated == 0:
                ...
                # 1. Iterate over pairs of nodes and check if we can allocate the demand
                while j < len(currentPath):
                    if new_state[env.edgesDict[str(currentPath[i]) + ':' + str(currentPath[j])]][0] - demand < 0 or \
                            new_state[env.nodesDict[str(currentPath[i])]][5] - demand < 0 or \
                            new_state[env.nodesDict[str(currentPath[j])]][5] - demand < 0:
                        can_allocate = 0
                        break
                    i = i + 1
                    j = j + 1
...
class DQNAgent:
    def __init__(self, env_nsfnet):
        ...
        self.bw_allocated_feature = np.zeros((40, 3))
        ...
        self.bw_demand_feature = np.zeros((40, 3))
...
def exec_dqn_model_episodes(experience_memory, env_dqn, agent):
    ...
    while iter_episode < len(experience_memory):
        if new_episode:
            ...
            action, state_action, beiyong, _ = agent.act(env_dqn, state, demand, source, destination, True)
            state1, reward, done, demand1, source1, destination1 = env_dqn.make_step(state, action, demand,
                                                                                     source, destination)
            if beiyong is not None:
                new_state, new_reward, new_done, new_demand, new_source, new_destination = env_dqn.make_step(
                    state, beiyong,
                    demand, source,
                    destination)
...
if __name__ == "__main__":
    ...
    data_files = ['./Logs/expsample_DQN_agentLogs.txt']

    aux = data_files[0].split(".")
    aux = aux[1].split("exp")
    ...
    plt.plot(rewards_sap, 'b', label="SPA")
    ...
    plt.title(topo, fontsize=16, fontweight='bold')
